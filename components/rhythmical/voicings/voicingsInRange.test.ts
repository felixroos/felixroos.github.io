...
describe('voicingsInRange', () => {
  ...
  test('tonaljs support of ireal chord symbols', () => {
    ...
    const realbook1chords = ["7", "-7", "^7", "7b9", "h7", "6", "-", "7sus", "M", "o7", "-6", "^7#11", "7#11", "-11", "-^7", "7#9", "7#5", "13", "7alt", "7b13", "^9", "sus", "o", "9sus", "-9", "9", "h", "-b6", "7#9#5", "13b9", "^7#5", "69", "7b5", "7b9sus", "7b13sus", "9#5", "13sus", "7b9b13", "7b9b5", "^9#11", "7#9b5", "9#11", "add9", "-69", "7susadd3", "9b5", "7b9#9", "5", "+", "13#11", "-7b5", "7b9#11"];
    ...
    expect(supported).toEqual(["7", "^7", "7b9", "h7", "6", "7sus", "M", "o7", "^7#11", "7#11", "7#9", "7#5", "13", "7alt", "7b13", "^9", "sus", "o", "9sus", "9", "h", "7#9#5", "13b9", "^7#5", "7b5", "7b9sus", "9#5", "13sus", "7b9b13", "7b9b5", "^9#11", "7#9b5", "9#11", "add9", "9b5", "7b9#9", "5", "+", "13#11", "7b9#11"])
    expect(unsupported).toEqual(["-7", "-", "-6", "-11", "-^7", "-9", "-b6", "69", "7b13sus", "-69", "7susadd3", "-7b5"])
    ...
    expect(unsupporedAlternatives).toEqual(
      ["69", "-b6", "7b13sus", "7susadd3"]
    ); // those chords will always fail
  ...
  test('aliases', () => {
    ...
    expect(Chord.get('C-7').aliases).toEqual(['M', '^', '']) // problem

    const tokenizeChord = (name) => {
      const [letter, acc, oct, type] = tokenizeNote(name);
      return
    }
    const noteRegex = /^([a-gA-G]?)(#{1,}|b{1,}|x{1,}|)(-?\d*)\s*(.*)$/;
    expect(noteRegex.exec('C7')).toEqual(['C7', 'C', '', '7', ''])
    expect(noteRegex.exec('C-7')).toEqual([''])
  })

  test('C major triad inversions', () => {
    expect(voicingsInRange('C', { 'M': ['1M 3M 5P', '3M 5P 8P', '5P 8P 10M'] }, ['C3', 'C5'])).toEqual([
      ['C3', 'E3', 'G3'],
      ['C4', 'E4', 'G4'],
      ['E3', 'G3', 'C4'],
      ['E4', 'G4', 'C5'],
