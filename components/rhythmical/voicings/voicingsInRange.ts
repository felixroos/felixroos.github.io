...
export function voicingsInRange(chord, dictionary = lefthand, range = ['D3', 'A4']): string[][] {
  const { tonic, aliases } = Chord.get(chord);
  // find equivalent symbol that is used as a key in dictionary:
  const symbol = !aliases?.length ? chord : Object.keys(dictionary).find(_symbol => aliases.includes(_symbol));
