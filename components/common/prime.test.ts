...
import { addVectors, fractionPrimeVector, nPrimes, primefactors, primelimit, primepowers, primes, primevector, ratioPrimePowers, ratioPrimeVector, slicePrimeVector, withoutFactor } from './prime'
...
test('primepowers', () => {
  expect(primepowers(66)).toEqual([[2, 1], [3, 1], [11, 1]]);
  expect(primepowers(15)).toEqual([[3, 1], [5, 1]]);
  expect(primepowers(12)).toEqual([[2, 2], [3, 1]]);
...
test('ratioPrimePowers', () => {
  expect(ratioPrimePowers(3, 2)).toEqual([[2, -1], [3, 1]]);
  expect(ratioPrimePowers(4, 3)).toEqual([[2, 2], [3, -1]]);
});
